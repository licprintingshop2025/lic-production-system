...
import { appendNonBIROrderRow, getNonBIROrderRows } from "@/lib/googleSheets";
...
function classifyPrintingStatus(listName: string) {
  ...
  if (normalized.includes("NON-BIR INTAKE") || normalized.includes("STATION 4")) return "Intake" as const;
...
export async function GET() {
  try {
    ...
    const orders = rows
      .slice(1)
      .map((row, index) => {
        const cardId = String(row[7] || "").trim();
        ...
        return {
          ...
          submittedAt: String(row[1] || "").trim(),
          businessName: String(row[2] || "").trim(),
          orderSummary: String(row[3] || "").trim(),
          quantitySummary: String(row[4] || "").trim(),
          salesAssigned: String(row[6] || "").trim(),
          trelloCardId: cardId,
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Failed to load Non-BIR printing orders.",
